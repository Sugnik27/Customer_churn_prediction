...
from src.config import CLEANED_DATA_PATH, MODEL_DIR, BEST_MODEL_PATH, FEATURES_PATH, CV_FOLD, SCORING, N_JOBS
...
def train_and_select_model() -> pd.DataFrame:
    ...
    print("Loading pre-cleaned data ->", CLEANED_DATA_PATH)
    ...
    for name, estimator, param_grid in models_and_params:
        print(f"\n=== Training {name} ===")
        pipeline = Pipeline([("preprocess", preprocess), ("clf", estimator)])

        grid = GridSearchCV(estimator=pipeline, param_grid=param_grid, cv=CV_FOLD, scoring=SCORING, n_jobs=N_JOBS, verbose=1)
